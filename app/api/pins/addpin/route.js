...
export async function POST(req) {
  try {
    ...
    for (const pinId of pinIds) {
      try {
        // First check if pin already exists - use exact matching with raw query
        const existingPins = await prisma.$queryRaw`
          SELECT * FROM "Pin" WHERE "pinId" = ${pinId}
        `;
        ...
        if (existingPin) {
          ...
          console.log(`Pin with pinId ${pinId} already exists in collection`);
