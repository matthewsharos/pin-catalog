...
export async function scrapePinDetails(pinId) {
  try {
    ...
    console.log(`Scraping pin from ${url}...`);
    ...
    if (!response.ok) {
      ...
      if (response.status === 404) {
        throw new Error(`Pin ID ${pinId} not found on Pin&Pop`);
      } else if (response.status === 429) {
        throw new Error('Rate limited by Pin&Pop. Please try again in a few minutes.');
      } else if (response.status >= 500) {
        throw new Error('Pin&Pop service is currently unavailable. Please try again later.');
      }
      throw new Error(`Failed to fetch pin data: ${response.status} ${response.statusText}`);
    }
    ...
    // If we still couldn't find the image, try to find it in the HTML source
    if (!thumbnailUrl) {
      const imagePattern = new RegExp(`https://pinandpop\\.s3\\.amazonaws\\.com/images/pinails/${pinId}_[A-Za-z0-9]{4}_pinail\\.(webp|jpg)`);
    ...
    if (!imageUrl) {
      ...
      imageUrl = imageUrl.replace('_pinail', '').replace('/pinails/', '/pins/');
    }
    ...
    console.error('Error scraping pin details:', error);
    throw error;
  }
}
